...
public class ProductRestController {
	...
	public ResponseEntity<Integer> readProduct(@PathVariable("name") String name) {
		...
		int price = 20000;
		// ResponseEntity : HTTP 응답의 상태 코드, 헤더 및 본문(body)을 포함하여 전송
	...
	// ㄴ 제품 데이터를 생성
	@PostMapping
	public ResponseEntity<Integer> createProduct(@RequestBody ProductDTO dto) {
		log.info("createExample()");
		log.info(dto);
		
		int result = 1; // 등록 성공에 대한 리턴값 예시
		return new ResponseEntity<Integer>(result, HttpStatus.OK);
	} // end createProduct()
	
} // end ProductRestController
